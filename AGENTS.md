<!-- repository: packages/ts/grpc | kind: PACKAGE | stack: ts-package -->

# grpc

> Repository development instructions for **grpc** (Shared Package).
> Stack: ts-package. Generated as part of the Omnixys repository-policy rollout.

## Repository Overview

Omnixys gRPC helpers for TypeScript.

- Repository path: `packages/ts/grpc` (relative to the Omnixys root)
...
src/ gRPC helpers + src/proto (copied to dist in postbuild)
...
Proto files are copied into dist; regenerate via the repository's mechanism, never edit generated output.
